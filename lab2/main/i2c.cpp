...
uint8_t I2C_getByte(bool moreDataLeft) {
    ...
    delayMicroseconds(350); // The bus seems to break if TWDR is read too quickly: stretch the clock/wait so that the register can be loaded (350us seems to be enough to prevent crashes).
    data = TWDR;
    if(!moreDataLeft) {
